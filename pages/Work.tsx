    ...
    <section className="relative h-screen w-full flex flex-col items-center justify-center text-center px-6 overflow-hidden">
      <div className="z-10 flex flex-col items-center">
        ...
        <motion.p
          ...
          className="text-lg md:text-xl text-[#86868b] max-w-[600px] font-medium leading-relaxed transition-colors duration-700"
        >
      ...
      >
        <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-[#86868b]">Scroll</span>
      ...
      <motion.div style={{ scale, opacity }} className="relative w-full max-w-[1200px] px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <motion.div style={{ y: textY }} className="space-y-8">
            ...
            <p className="text-lg md:text-xl text-[#86868b] font-medium leading-relaxed max-w-md transition-colors duration-700">
              {project.description}
            </p>
            {project.link ? (
              ...
              <div className="inline-flex items-center space-x-3 px-10 py-4 bg-[#f5f5f7] dark:bg-white/5 text-[#86868b] rounded-full font-bold text-sm cursor-not-allowed">
          ...
          <motion.div className="relative group cursor-pointer overflow-hidden rounded-[40px] shadow-2xl border border-black/[0.03] dark:border-white/[0.05]">
            <motion.div 
            ...
            >
              <div className="glass px-8 py-4 rounded-full border border-white/20 dark:border-white/10 shadow-2xl flex items-center space-x-3">
                <span className="text-[11px] font-bold tracking-widest uppercase text-[#1D1D1F] dark:text-white">View Case Study</span>
  ...
  return (
    <section className="py-48 max-w-[1200px] mx-auto px-6">
      <div className="text-center mb-24 space-y-6">
        ...
        <p className="text-xl text-[#86868b] font-semibold transition-colors duration-700">Small but meaningful contributions to the industry.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 auto-rows-[420px]">
        {items.map((item, i) => (
          <div 
            key={i} 
            className={`${item.span} relative group overflow-hidden rounded-[40px] border border-black/[0.04] dark:border-white/[0.1] bg-[#050505] shadow-2xl transition-all duration-700 cursor-default flex flex-col`}
          >
            ...
            <div className={`relative z-10 p-10 flex flex-col h-full ${item.align === 'bottom' ? 'justify-end' : 'justify-start'}`}>
              <motion.h3 
                ...
                className="text-4xl md:text-[42px] font-extrabold tracking-apple mb-3 text-white leading-[1.1]"
              >
                {item.title}
              </motion.h3>
              <motion.p 
                ...
                className="text-[#86868b] text-lg font-medium leading-relaxed max-w-[90%] group-hover:text-white/80 transition-colors"
              >
                {item.desc}
              </motion.p>
            </div>
